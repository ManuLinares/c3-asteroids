...
case $TARGET in
    ...
    windows)
        ...
        mkdir -p "$OUT_DIR"
        # We use clang to cross-compile for windows
        clang -O3 -target x86_64-pc-windows-gnu -mno-stack-arg-probe -D__USE_MINGW_ANSI_STDIO=0 -x c -c "$MA_HEADER" -o "$OUT_DIR/miniaudio.obj" $MA_DEFS -DMA_NO_WEBAUDIO
        ;;
    *)
